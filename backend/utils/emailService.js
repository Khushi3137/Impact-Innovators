...
exports.sendGroupInviteEmail = async ({ email, group, inviter }) => {
  const frontendUrl = process.env.FRONTEND_URL || 'http://localhost:5173';
  const joinUrl = `${frontendUrl}/groups?groupId=${group._id}`;
  const inviterName = inviter?.name || inviter?.email || 'A classmate';
  ...
  const mailOptions = {
    ...
    html: `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
        <h2 style="color: #1976d2;">Study Group Invitation</h2>
        <p>Hi,</p>
        <p>${inviterName} invited you to join the study group <strong>${group.name}</strong>.</p>
        <div style="text-align: center; margin: 30px 0;">
          <a href="${joinUrl}" style="background-color: #1976d2; color: white; padding: 12px 24px; text-decoration: none; border-radius: 5px; font-weight: bold;">
            Join Group
          </a>
        </div>
        <p>Or copy and paste this link in your browser:</p>
  ...
  try {
    await transporter.sendMail(mailOptions);
    console.log(`Group invite sent to ${email}`);
    return true;
  } catch (error) {
